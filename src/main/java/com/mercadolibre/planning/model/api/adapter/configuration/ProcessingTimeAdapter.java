...
public class ProcessingTimeAdapter {
  ...
  public List<EtdProcessingTimeData> getOutboundProcessingTimeByCptInRange(
    ...
    return getSortedOutboundProcessingTimesInRangeByDayAndHour(processingTimesInDays, zonedDateTimeFrom, zonedDateTimeTo);
  }

  /**
   * Retrieves a list of OutboundProcessingTime objects based on active routes from the route client.
   *
   * @param logisticCenterId The ID of the logistic center.
   * @return A list of OutboundProcessingTime objects corresponding to the active routes of the logistic center.
   * @throws ProcessingTimeException if something happens when executing the Api Client request.
   */
  public List<EtdProcessingTimeData> getOutboundProcessingTimeByLogisticCenterFromRouteClient(final String logisticCenterId) {
  ...
  /**
   * Updates OutboundProcessingTime records for a logistic center with the provided processing times.
   *
   * @param logisticCenterId The ID of the logistic center.
   * @param processingTimes  The list of OutboundProcessingTime objects to update.
   * @return A list of updated OutboundProcessingTime objects for the logistic center.
   * @throws ProcessingTimeException if something happens when executing a repo method.
   */
  public List<EtdProcessingTimeData> updateOutboundProcessingTimesForLogisticCenter(
      final String logisticCenterId,
      final List<EtdProcessingTimeData> processingTimes
  ) {
    final List<OutboundProcessingTime> processingTimesToSave = processingTimes.stream()
        .map(EtdProcessingTimeData::toActiveOutboundProcessingTime)
        .toList();
   ...
   */
  private List<EtdProcessingTimeData> getSortedOutboundProcessingTimesInRangeByDayAndHour(
      final List<OutboundProcessingTime> processingTimes,
    ...
    return toEtdProcessingTimeData(processingTimesInDateRange);
  }
  ...
  private List<EtdProcessingTimeData> getSortedOutboundProcessingTimesByDayAndHour(
      final List<RouteEtsDto> activeRoutes,
      final String logisticCenterId
  ) {

    final Collection<EtdProcessingTimeData> distinctProcessingTimesByDay = createDistinctProcessingTimesByDayMapFromRouteDto(
        activeRoutes,
        logisticCenterId
    );
  ...
  private Collection<EtdProcessingTimeData> createDistinctProcessingTimesByDayMapFromRouteDto(
      final List<RouteEtsDto> activeRoutes,
      final String logisticCenterId
  ) {
    ...
    return allCptDays
        .map(day -> toEtdProcessingTimeData(day, logisticCenterId))
        .collect(
            Collectors.toMap(
  ...
  private EtdProcessingTimeData toEtdProcessingTimeData(final DayDto dayDto, final String logisticCenterId) {
    ...
    return new EtdProcessingTimeData(
        logisticCenterId,
        dayDto.getEtDay(),
        dayDto.getEtHour(),
  ...
  private List<EtdProcessingTimeData> toEtdProcessingTimeData(final Stream<OutboundProcessingTime> processingTimeData) {
