...
public class RouteCoverageClient extends HttpClient implements RouteCoverageClientGateway {
   ...
   * Se asume que esto no ocurre porque el flowmonitor reliza llamadas periódicamente a intervalos menores al tiempo del {@link #ç}.
   */
  @Trace
  @Override
  public List<RouteCoverageResult> get(final String warehouse) {
    ...
    if (hit == null) {
      ...
      cache.put(warehouse, new Record(value, currentNano + TTL.toNanos()));
      return value;
    } else {
  ...
  private void refresh(final String warehouse) {
    ...
    CompletableFuture.runAsync(
        () -> {
          ...
          final var value = load(warehouse);
          cache.put(warehouse, new Record(value, System.nanoTime() + TTL.toNanos()));
        },
        refreshExecutor
    );
  }
