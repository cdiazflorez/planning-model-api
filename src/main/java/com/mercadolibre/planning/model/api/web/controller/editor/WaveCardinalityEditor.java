...
public class WaveCardinalityEditor extends PropertyEditorSupport {
    ...
    public void setAsText(final String text) {
        ...
        final WaveCardinality waveCardinality = WaveCardinality.of(text).orElseThrow(() -> {
            final String message = format("Value %s should be one of %s",
                    text,
                    Arrays.toString(WaveCardinality.values()));

            return new IllegalArgumentException(message);
        });

        setValue(waveCardinality);
    }
}
